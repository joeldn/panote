...
| `workers.json` | Cloudflare Workers (`services/*`) | `base.json` | `types: []` (no `node` types — workerd isn't Node; ambient Worker types come from the package's own generated `worker-configuration.d.ts`); `module: ESNext` / `moduleResolution: bundler`; `noEmit: true` — Wrangler/esbuild does the bundling, so `tsc` is type-check-only |
...
- **Worker runtime types are generated per-Worker by `wrangler types`, not taken from `@cloudflare/workers-types`** — Cloudflare superseded that package, and the generated `worker-configuration.d.ts` is binding-exact (a Durable Object namespace is typed `DurableObjectNamespace<TheClass>`, and only the vars that actually exist are declared). Every `services/*` package and `@internal/worker-kit` commits its generated `worker-configuration.d.ts` and carries a `types` script to regenerate it (`wrangler types --env dev --strict-vars=false`). Gotchas: (1) the file must be listed in the package's tsconfig `include` or none of the runtime globals resolve; (2) it must be regenerated whenever a binding, var, `compatibility_date` or `compatibility_flag` changes — `wrangler types --check` verifies it without rewriting; (3) it is generated per `--env`, so it is generated from `dev` and `--strict-vars=false` keeps env-specific literal values out of the type — never compare a var against a string literal; (4) it is excluded from the Prettier gate (it is tab-indented generated output) and self-excludes from ESLint via a leading `/* eslint-disable */`; (5) the test-only pool types (`@cloudflare/vitest-pool-workers/types`) go in `tsconfig.test.json`, and it must be the `/types` subpath — the bare package name yields `TS2305: Module '"cloudflare:test"' has no exported member 'SELF'`
